...
import { Layout, Menu, Map, Settings, LogOut, Sun, Globe, X } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    // 1. Get current hostname (Real world scenario)
    ...
    if (matchedBrand) {
        // If matched, we lock the view to 'shop' for that brand
        setSelectedBrand(matchedBrand);
        setViewMode('shop');
    } else {
        // If no match (e.g. localhost or admin domain), show landing
        setViewMode('landing');
    }
    ...
    return (
      <>
        ...
        <CustomerPortal 
            ...
            onBackToLanding={() => {
                // If simulating a domain, "Back" implies leaving the site, so we clear simulation
